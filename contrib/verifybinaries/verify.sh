...
###   icomexcore.org and icomex.org and compares them.
...
WORKINGDIR="/tmp/icomex_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://icomexcore.org"
HOST2="https://icomex.org"
BASEDIR="/bin/"
VERSIONPREFIX="icomex-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'icomex-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "icomex.org failed to provide signature file, but icomexcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "icomex.org and icomexcore.org signature files were not equal?"
