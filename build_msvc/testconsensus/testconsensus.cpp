...
// icomex includes.
#include <..\src\script\icomexconsensus.h>
...
int main()
{
    std::cout << "icomexconsensus version: " << icomexconsensus_version() << std::endl;
    ...
    auto op0Result = icomexconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, icomexconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
