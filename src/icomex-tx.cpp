...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " icomex-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  icomex-tx [options] <hex-tx> [commands]  Update hex-encoded icomex transaction\n" +
            "or:     icomex-tx [options] -create [commands]   Create hex-encoded icomex transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded icomex transaction
