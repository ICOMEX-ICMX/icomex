...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. icomex_de.qm (shortcut "de" needs to be defined in icomex.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. icomex_de_DE.qm (shortcut "de_DE" needs to be defined in icomex.qrc)
...
static const char* qt_argv = "icomex-qt";
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // icomex: URIs or payment requests:
        if (paymentServer) {
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(icomex);
    Q_INIT_RESOURCE(icomex_locale);
    ...
    /// 6. Determine availability of data directory and parse icomex.conf
    ...
    // icomex: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
